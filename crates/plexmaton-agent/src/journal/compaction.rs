use plexmaton_core::{AgentId, HeadName, SessionEntryId};

use super::{JournalEntryPayload, JournalError, SessionJournal};
use crate::{
    CompactionAttemptFinished, CompactionCheckpoint, CompactionOutcome, CompactionSource,
    ContextAtom, ContextAtomValue, ContextEpoch, RequestAttemptAuthorized, RequestAttemptOwner,
};
...
impl SessionJournal {
    ...
    pub(super) fn checkpoint_replacement(
        ...
        atoms: &[ContextAtom],
        checkpoint: &CompactionCheckpoint,
        checkpoint_entry_id: SessionEntryId,
        ...
        if cut.first_retained <= usize::from(cut.pinned_user.is_some()) {
            return Err(JournalError::CompactionMakesNoProgress);
        }
        let mut replacement = Vec::with_capacity(
            1_usize
                .saturating_add(usize::from(cut.pinned_user.is_some()))
        ...
        if let Some(pinned) = cut.pinned_user {
            replacement.push(atoms[pinned].clone());
        }
...
fn resolve_cut(
    ...
    let covered_latest_user = latest_user.filter(|index| *index <= last);
    let pinned_user = match (cut.pinned_user(), covered_latest_user) {
        (Some(expected), Some(index)) if expected == first_ids[index] => Some(index),
        (None, None) => None,
        _ => return Err(JournalError::InvalidCompactionCut),
    };
    Ok(ResolvedCut {
...
fn epoch_from_path(path: &[&super::SessionEntry]) -> ContextEpoch {
    path.iter()
        .rev()
        .find_map(|entry| {
            matches!(
