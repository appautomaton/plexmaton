use plexmaton_core::{SessionEvent, TranscriptItemId};
...
mod tests {
    use plexmaton_core::{
        AgentId, HeadName, JournalRecordId, SessionEntryId, SessionEvent, TokenCounts, TokenUsage,
    };
    ...
    fn assert_checkpoint_context(
        ...
        events_before: &[plexmaton_core::SessionEventEnvelope],
    ) {
    ...
    fn huge_tool_context() -> (Agent, Vec<crate::ContextAtom>) {
        ...
        assert!(matches!(running.effects.as_slice(), [Effect::RunTool(_)]));
    ...
    fn failed_attempt_keeps_the_frozen_source_usable() {
        ...
        assert!(reaction.events.iter().any(|event| matches!(
            &event.event,
            SessionEvent::RuntimeError { message, .. } if message == "compaction is unavailable"
        )));
        assert_eq!(
