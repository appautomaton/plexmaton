    ...
    pub fn new(
        ...
        if let Some(replay) = &replay {
            for attachment in replay.attachments() {
                ...
                if !matches!(block, AssistantBlock::Reasoning { .. }) {
                    return Err(ContextError::ReplayAnchorNotReasoning);
                }
            }
        }
...
impl std::fmt::Display for ContextError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self {
            ...
            Self::ReplayAnchorNotReasoning => {
                "assistant replay anchor does not name a reasoning block"
            }
