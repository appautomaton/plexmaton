...
/// The latest user atom and its directly attached explicit skill are required context (CPL-3,
/// SKL-5). Valid journal projection places at most one skill immediately after its owning user.
#[must_use]
pub fn required_user_context(atoms: &[crate::ContextAtom]) -> Option<std::ops::Range<usize>> {
    let start = atoms
        .iter()
        .rposition(|atom| matches!(atom.value(), crate::ContextAtomValue::User { .. }))?;
    let attached = atoms
        .get(start + 1)
        .is_some_and(|atom| matches!(atom.value(), crate::ContextAtomValue::Skill(_)));
    Some(start..start + 1 + usize::from(attached))
}
...
pub enum ContextEpoch {
    Original,
    Checkpoint(ConversationEntryId),
}
...
impl CompactionSource {
    ...
    #[must_use]
    pub const fn boundary(&self) -> &ConversationEntryId {
        &self.boundary
    }
...
pub struct CompactionCut {
    first_covered: ConversationEntryId,
    last_covered: ConversationEntryId,
    first_retained: Option<ConversationEntryId>,
    pinned_user: Option<ConversationEntryId>,
}
...
impl CompactionCut {
    ...
    #[must_use]
    pub const fn first_covered(&self) -> &ConversationEntryId {
        &self.first_covered
    }

    #[must_use]
    pub const fn last_covered(&self) -> &ConversationEntryId {
        &self.last_covered
    }

    #[must_use]
    pub const fn first_retained(&self) -> Option<&ConversationEntryId> {
        self.first_retained.as_ref()
    }

    #[must_use]
    pub const fn pinned_user(&self) -> Option<&ConversationEntryId> {
...
#[cfg(test)]
mod tests {
    use plexmaton_core::{ConversationEntryId, HeadName, TokenUsage, TranscriptItemId};
    ...
    fn plan(max_summary_bytes: usize) -> Result<CompactionPlan, CompactionPlanError> {
        CompactionPlan::new(
            ...
            CompactionSource::new(
                ...
                HeadRevision::new(2),
                id("boundary", ConversationEntryId::new),
                ContextEpoch::Original,
            ),
            CompactionCut::new(
                id("first", ConversationEntryId::new),
                id("last", ConversationEntryId::new),
                None,
                Some(id("user", ConversationEntryId::new)),
            ),
