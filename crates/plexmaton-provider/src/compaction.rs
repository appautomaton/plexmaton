...
use plexmaton_agent::{
    ...
    CompactionPlanError, CompactionSource, ContextAtom, ContextAtomValue,
    MAX_COMPACTION_SUMMARY_BYTES, ModelRequest, SessionJournal, TokenEstimate,
};
...
pub enum CompactionPreparationError {
    ...
    #[error("the latest required user atom exceeds available input capacity")]
...
pub fn validate_compaction_output(
    ...
    {
        atoms.push(prepared.frozen_request.atoms[index].clone());
    }
    atoms.extend(
...
fn prepare(
    ...
    let latest_user = atoms
        .iter()
        .rposition(|atom| matches!(atom.value(), ContextAtomValue::User { .. }));
    if let Some(index) = latest_user
        && required_user_is_oversized(model, tools, &budgeted.request, &atoms[index], capacity)?
    {
    ...
    let suffix_start = select_suffix_start(&budgeted, suffix_limit, latest_user);
    if suffix_start == 0 {
        return Err(CompactionPreparationError::NoUsefulReduction);
    }
...
fn append_instruction(request: &ModelRequest, source: SessionEntryId) -> ModelRequest {
...
fn atom_id(atom: &ContextAtom) -> SessionEntryId {
    atom.source_entries()
        .first()
        .cloned()
...
fn index_of(request: &ModelRequest, id: &SessionEntryId) -> Option<usize> {
    request
        .atoms
        .iter()
