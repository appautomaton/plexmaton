//! Immutable request cost calculated from one resolved price table and complete usage.
...
/// Calculates the fixed cost stored with one request terminal.
///
/// Missing pricing, partial usage, inconsistent subsets, or an unrepresentable amount stays
...
pub fn request_cost(model: &ResolvedModel, usage: &TokenUsage) -> RequestCost {
    ...
    let TokenUsage::Complete(counts) = usage else {
        return RequestCost::Unavailable;
    };
...
fn calculate(prices: &ModelCost, counts: &TokenCounts) -> Option<u64> {
    if counts.input.checked_add(counts.output)? != counts.total
        || counts.reasoning_output? > counts.output
    {
        return None;
    }
