...
use plexmaton_agent::{
    ...
    CompactionOutcome, CompactionPlan, ContextAtomValue, ContextEpoch, DispatchedRequestTiming,
    ElapsedMillis, Input, JournalRecord, JournalSequence, ModelEvent, ProviderCodecId,
    ProviderCodecRevision, ProviderModelFamilyId, ProviderReplay, ProviderReplayOwnerId, Reaction,
    ReplayCompatibility, RequestAccounting, RequestAttemptOwner, RequestAttemptTerminal,
    RequestAttemptTerminalState, RequestCost, RequestDispatchedOutcome, RequestEnvironment,
    RequestEnvironmentFingerprint, SessionJournal, SessionMetadata, StopReason, TurnBudget,
    UnixMillis,
};
use plexmaton_core::{
    AgentId, HeadName, JournalRecordId, SessionEntryId, SessionEvent, SessionId, TokenUsage,
    TranscriptItemId,
};
...
// Captured through public APIs while the worktree base was 2bb0a70f56660d95d4feb8f11ec7ead3336db5a7.
...
fn assert_checkpoint_projection(journal: &SessionJournal) {
...
fn assert_compaction_audit(journal: &SessionJournal) {
...
fn failed_compaction_diagnostic_reopens_without_exposing_partial_output() {
    ...
    let session_id = id("failed-compaction-session", SessionId::new);
    let metadata = SessionMetadata::new(session_id.clone(), UnixMillis::new(500));
    ...
    for selected in [
        ...
        let diagnostics: Vec<_> = projection
            .events()
            .iter()
            .filter(|event| matches!(event.event, SessionEvent::RuntimeError { .. }))
            .collect();
