...
use plexmaton_agent::{
    AssistantBlock, ContextAtomValue, HeadRevision, JournalEntryPayload, JournalProjection,
    JournalRecord, ModelRequest, RequestAccounting, RequestCost, SessionEntry, SessionJournal,
    ToolOutcome, UnixMillis, UsdCostTicks,
};
use plexmaton_core::{
    AgentId, HeadName, JournalRecordId, SessionEntryId, SessionId, TokenCounts, TokenUsage,
...
// Captured through JournalFile's public API at 2bb0a70f56660d95d4feb8f11ec7ead3336db5a7.
...
fn assert_journal_shape(journal: &SessionJournal, main: &HeadName, branch: &HeadName) {
    assert_eq!(
        journal.session_id(),
        &id("pre-compaction-session", SessionId::new)
    );
    assert_eq!(
    ...
    assert_eq!(
        journal
            .head_target(main)
            .map(|target| target.map(SessionEntryId::as_str)),
        Ok(Some("entry-13"))
    );
    assert_eq!(
        journal
...
fn append_continuation(file: &mut JournalFile, main: &HeadName) {
    ...
    file.append(JournalRecord::AppendEntry {
        ...
        entry: Box::new(SessionEntry {
            id: id("entry-continuation", SessionEntryId::new),
            parent_id: file
                .journal()
                .head_target(main)
...
/// JRN-3/JRN-5/TIM-3: the accepted pre-compaction wire remains readable and continuable.
#[test]
