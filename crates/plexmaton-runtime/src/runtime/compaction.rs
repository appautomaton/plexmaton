...
type RunningCompaction<'a> = (
    &'a mut RetainedFuture<CompactionAttemptFinished>,
    &'a mut Pin<Box<Sleep>>,
    bool,
);
...
impl LiveRuntime {
    ...
    pub(super) fn running_compaction(&mut self) -> Option<RunningCompaction<'_>> {
        let operation = self.compaction.as_mut()?;
        ...
        Some((future, deadline, *timed_out))
    }
