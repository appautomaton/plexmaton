...
use plexmaton_agent::{
    ...
    CompactionId, CompactionInputMode, CompactionOutcome, ContextEpoch, DispatchedRequestTiming,
    ElapsedMillis, Input, JournalRecord, ModelEvent, ModelOutputPosition, ProviderReplay,
    RequestAttemptTerminal, RequestAttemptTerminalState, RequestCost, RequestDispatchedOutcome,
    SessionJournal, StopReason, UnixMillis,
};
use plexmaton_core::{
    AgentId, HeadName, JournalRecordId, SessionEntryId, TokenUsage, TranscriptItemId,
};
...
fn fork(file: &mut JournalFile, name: &str, target: SessionEntryId) -> HeadName {
...
fn cpl_5_retention_config_change_preserves_checkpoint_and_continuation_bytes() {
    ...
    for api in [
        ...
        let mut file = JournalFile::create(
            &path,
            expected_journal.session_id().clone(),
            UnixMillis::EPOCH,
        )
        .expect("journal");
