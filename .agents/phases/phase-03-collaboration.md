...
| Field | Value |
| --- | --- |
| Status | Active; stage 7 and stage 8 provisioning recovery are complete; stage 8 slice 2 is next |
...
checkpointed locally through `a5cfdb2`; stage 8.1 is complete on this task branch. Earlier
...
| Persistence | The smoke reads child JSONL and checks delegation/mail/Handoff event kinds. Graceful root resume restores the exact child history and User controller without a provider request or durable write. A real child-process witness now kills the provisioning owner after canonical creation, child-journal creation, runner registration and acknowledged mail; each cut reopens one exact delegation, preserves retry receipts/mail and recovers one passive child. Full CLI kill→resume projection equality remains unproven |
...
| [8 — Recovery acceptance](../plans/phase-03-stage-08-recovery-acceptance.md) | Provisioning crash cuts; actual CLI kill→resume; pending approval readmission; final acceptance and closure | Active; slice 1 complete, slice 2 next |
...
| Background requests have one canonical source, preserve focus, and resolve only through their owner | Canonical live routing, passive replay and graceful-shutdown resolution pass; the CHB-1 production request source, real PTY journey and recovered continuation remain | 7.7, 8.2, 8.3 |
...
- **Approval recovery:** JRN-5 currently settles orphaned calls as interrupted; APV-6 requires fresh
  ...
  treating cancellation as readmission is permitted. That slice also owns the first request source
  supported by the CHB-1 production child profile and its real 120/95/60 PTY journey; a synthetic
  policy or false tool capability is not acceptance evidence.
