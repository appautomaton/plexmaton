...
3. **Context atoms and replay compatibility (active).** Project one selected head into typed message,
   ...
   compatible ordering.
4. **Request attempts and immutable usage.** Implement TIM-2–TIM-5 with agent-step and compaction
...
5. **Budget ledger.** Give every atom and request-environment input a deterministic estimated cost;
...
6. **Pure compaction plan.** Select a covered prefix and byte-exact retained suffix in atom units;
...
7. **Durable checkpoint.** Add one versioned checkpoint payload and projection rule. Commit its
...
8. **Bounded automatic orchestration.** Invoke compaction before a turn at the soft threshold,
...
9. **Manageable heads and rewind journey.** Expose create, select, rename, abandon and rewind over
...
The ledger measures pressure before compaction freezes a payload. Checkpoints precede automatic
triggers, so recovery exists first. Rewind composes the earlier boundaries and needs user review.
