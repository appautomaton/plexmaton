...
| PER-3 | `per_3_file_change_preset_pins_definitions_and_excludes_control_paths`, `per_3_inspection_preset_pins_only_read_and_search_definitions`; existing native admission/executor tests remain APV-3 evidence; `a_binding_carries_the_whole_path_after_one_equals` pins the confined-command definition |
| PER-11 | `cmd_7_a_confined_command_runs_without_asking`, `cmd_7_revoking_the_preset_brings_the_question_back`, `per_3_a_seeded_preset_writes_the_project_and_asks_for_the_control_plane`, `per_7_session_setting_releases_native_waiters_but_leaves_commands_pending`, `per_7_session_setting_before_first_turn_survives_new_and_revokes_without_jsonl` |
