...
it reads only piped requests and returns framed data, never terminal commands (MD-4/MD-5).
...
| Request | 16 entries; 192 KiB of snapshot admission before cloning, 256 KiB encoded; length prefix checked before allocation |
| Reply | 2 MiB encoded bytes and aggregate prepared allocation; oversized batches return a typed refusal |
| Prepared entry | 1 MiB allocation, 8,193 rows; 4 KiB identity admission; validated UTF-8 copy ranges, row widths, atomic rectangle/run consistency and selection-padding bounds |
