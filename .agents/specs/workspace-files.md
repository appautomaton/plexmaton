...
| Field | Value |
| --- | --- |
| Status | Proven through Phase 01 stage 2 slice 8 |
...
omitted defaults into one canonical form. Read and search declare only `FileRead`, and the executor
dispatches by pinned definition identity and revision rather than by the model's tool name.
...
| WFS-3 | `successful_reads_issue_bounded_observations`, `a_change_before_the_version_check_refuses_the_read`, `the_observed_range_excludes_skipped_and_lookahead_bytes`, `an_observation_authorizes_only_ranges_inside_its_read_window` |
