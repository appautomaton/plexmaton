...
| Field | Value |
| --- | --- |
| Status | Proven for Phase 01 stage 2 slice 7 |
| Owns | Workspace path authority, bounded exact text reads, observed file versions, and bounded ripgrep search |
| Depends on | [tool-admission](./tool-admission.md) APV-1 through APV-3; Phase 01 §producer |
| Proven by | `plexmaton-file-tools` component and integration tests named below |
...
names the normalized path and descriptor metadata observed before and after the read; a concurrent
change refuses the result. The registry has a hard entry bound, and Slice 8 must resolve the ID and
recheck its version rather than trusting a model-supplied hash.

**WFS-4 — Search is argv-based and acquisition-bounded.** Trusted executables must be absolute.
Ripgrep runs with configuration and symlink following disabled; pattern and glob are distinct
arguments while path selects the pinned descriptor rather than entering a shell. Candidate count,
...
invalid windows and oversized strings are refused before execution; read and search declare only
`FileRead`, and the executor dispatches by pinned definition identity and revision rather than by
