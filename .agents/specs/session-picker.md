...
Restoration never submits input or dispatches model/tool work. The exit handoff names the selected
session, not the session that happened to open the process.
...
| Invariant | Proven by |
| --- | --- |
| SPK-1 | `resume_aliases_share_one_command_and_retry_is_not_a_global_command`, `listing_is_bounded_read_only_and_rejects_symlinks`, `session_picker_keyboard_and_mouse_share_identity_and_cancel_drags`, `session_picker_frames_cover_empty_populated_and_failure_states`, `short_session_picker_keeps_selected_result_and_footer_visible`, `short_command_palette_and_wheel_use_the_visible_choice_window` |
| SPK-2 | `session_switch_validates_before_replacing_and_never_dispatches`, `cancelled_picker_releases_candidate_and_preserves_current_draft` |
| SPK-3 | `cancelled_picker_releases_candidate_and_preserves_current_draft` |
