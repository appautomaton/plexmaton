...
display labels and prompt prose are never authority.
...
| Boundary | Slice 4 | Later |
| --- | --- | --- |
| Tool catalog | Admission interface and test definitions | Native tools and MCP definitions declare schemas, capabilities and bounded details |
| Approval policy | Stateless policy over the admitted call | Phase 02 may add session- and workspace-scoped grants behind a durable policy store |
...
| Executor | Receives only admitted, allowed calls | Filesystem, process and network adapters enforce their own hard constraints |
