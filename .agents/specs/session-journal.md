...
| Field | Value |
| --- | --- |
| Status | Reducer, projections, JSONL adapter and commit owner implemented |
...
| Proven by | `plexmaton-agent::journal`, `plexmaton-session-store`, `plexmaton-runtime` and composition tests |
...
owner-only; Slice 6 owns the sessions directory. Rejected: per-record `fsync` or macOS
`F_FULLFSYNC` for a stronger power-loss promise; and a database or on-disk index before a measured
query need.
...
transition, gives its records to one bounded writer, and publishes its events or starts its effects
only after every append returns. A cancelled caller leaves that transition owned for the next poll;
shutdown joins every accepted append and the writer itself. A failed user transition returns the
exact direct or queued text in process-local arrival order, distinguishes definitely-unwritten from
outcome-unknown storage, shows one notice, joins all outside owners visibly, and requires reopen
before more work. The TUI loop performs no filesystem operation.
