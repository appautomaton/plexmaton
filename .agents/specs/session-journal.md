...
| Field | Value |
| --- | --- |
| Status | Implemented through Phase 02 stage 2 slice 2 |
...
mutate nothing; turn terminals additionally reject missing, duplicate or mismatched ownership.

**JRN-3 — The wire is lossless without weakening opaque replay.** Records use tagged JSON decoded
through validating constructors. Serialization retains exact `ProviderReplay` bytes;
`Debug` and decode errors reveal none. The header names `plexmaton.session`, the `2026-09-04`
...
**JRN-5 — Replay performs no effects.** One head derives a `ModelRequest` and
