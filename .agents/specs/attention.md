...
| Field | Value |
| --- | --- |
| Status | Implemented for both routes: the primary's approvals answer inline, a background agent's are announced on its roster row |
...
are only working, and says what is wanted. The announcement costs the conversation no row, because
the roster is a panel that already exists and already names every agent.
...
| Counts | The conversation's activity line ends with a pill, `( !n )`, in the action-required role while `n` requests are unanswered, and nothing otherwise. It is chrome on a row that already exists, so it costs no row and is no focus stop. Rejected: the same count on the roster's own title, which put it on the panel furthest from where the user is reading, beside rows already carrying each agent's state |
...
| A terminal too narrow for the roster's column | The roster is a panel the user opens; closed, the pill still counts what is unanswered |
