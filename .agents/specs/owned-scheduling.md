...
| Field | Value |
| --- | --- |
| Status | Implemented, wired and accepted. `scripts/smoke-delegate.py` runs a child beside a responsive root, then stops a focused child at a paused provider boundary while root input continues |
...
use bounded typed commands and replies and cannot obtain mutable file/runtime access. Cancelling a
reply wait does not cancel an accepted command or discard its retained result, including regular
admission and Handoff before preflight acknowledgement.

**SCH-2 — Control retains reserved progress.** Normal scheduling, runtime updates, disposable
inspection and control use separate bounded lanes. Saturated normal/update traffic cannot prevent
stop or shutdown admission; inspection consumes no control capacity. Stop admission is synchronous
and owner-retained: repeating the same target is idempotent, while a different target receives one
typed in-progress refusal.
...
closes normal admission, settles queued and active work, and only then appends the durable Handoff.
Shutdown resumes internally retained schedule/Stop/Handoff operations without a caller token,
returns their exact results, joins every runner and then the writer even when quiescence fails.
