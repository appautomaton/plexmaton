...
projects its persisted summary, the latest user atom when it was covered, and the exact retained
...
Original entries and the existing `plexmaton.session` / `2026-09-04` grammar remain readable.
...
- `ContextEpoch`: original context or a checkpoint `SessionEntryId`; derived from ancestry.
...
content is not truncated. This policy controls checkpoint projection, never the summarizer input.
...
| CPL-3 | `cpl_3_replacement_preview_rejects_oversized_summary_without_mutating_source`, `equal_atom_count_checkpoints_replace_huge_batches_and_prior_summaries`, `checkpoint_publication_rechecks_all_frozen_provenance`, `cpl_3_planning_refusals_are_typed_and_leave_source_unchanged`, `cpl_3_replacement_preview_rejects_token_non_progress`, `cpl_3_configured_recent_tail_changes_only_the_checkpoint_cut`, `cpl_3_recent_tail_target_is_capped_by_available_input` |
