...
| SURF-2 | Unproven, and unowned inside Phase 00. **Correction:** delivery step 2 predicted step 8 would own this, on the grounds that a transcript item scrolled past its viewport edge is a surface outgrowing its parent. It is not — an item is content *inside* a surface, and which rows of it a frame builds is [transcript-layout](./transcript-layout.md) TR-2's business. Layout tiles the terminal, so nothing overlaps and nothing overflows; the first surface that does is one drawn outside the tiling, which no phase has yet needed |
